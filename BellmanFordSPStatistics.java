...
public class BellmanFordSPStatistics {
    public static void main(String[] args) {
        ...
        List<BellmanFordsSP> shortestPaths = new ArrayList<>();

        for (String datasetPath : datasetPaths) {
            In in = new In(datasetPath);
            EdgeWeightedDigraph G = new EdgeWeightedDigraph(in);
            BellmanFordsSP sp = new BellmanFordsSP(G, 0); // Assuming source vertex is 0
            graphs.add(G);
        ...
        for (int i = 0; i < graphs.size(); i++) {
            ...
            BellmanFordsSP sp = shortestPaths.get(i);
